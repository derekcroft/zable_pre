      ...
      module ClassMethods
        def inherited(subclass)
          subclass.class_eval do
            scope :for_sort_params, lambda { |sort_params| inject_sort_scope(sort_params) }
          end
          super(subclass)
        end
        ...
        def sortable(*attr_names)
          attr_names.each do |attr_name|
            scope "sort_#{attr_name}", lambda { |options| order("#{attr_name} #{options[:order]}") }
          end
        end
        ...
        def scope_for_sort_attribute(sort_params)
          ...
          sort_params['order'] = 'ASC' if sort_params['order'].nil? || sort_params['order'].to_s.upcase != "DESC"
